...
class EspCamWidget(QtWidgets.QWidget):
    ...
    def populate_ui_ctrl(self):
        ...
        self.ctrl_layout.addRow(self.save_button)
    ...
    def save_image(self):
        if hasattr(self, 'img'):
            filename, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save Image", "", "Images (*.png *.jpg *.bmp)")
            if filename:
                cv2.imwrite(filename, cv2.cvtColor(self.img, cv2.COLOR_RGB2BGR))
    ...
    def grab_image(self):
        ...
        # result = self.rpc_master.call("do_obj_detection", recv_timeout=1000)
        # print(str(datetime.now() - Tstart)  + ': ' + str(result))
        if result is not None:
            ...
            self.img = cv2.imdecode(np.frombuffer(self.buf, dtype=np.uint8), cv2.IMREAD_GRAYSCALE)
