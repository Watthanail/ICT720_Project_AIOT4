...
#define MQTT_SOUND_TOPIC "taist/aiot/sound/dev_8"  // pubic
#define MQTT_HB_TOPIC "taist/aiot/heartbeat/dev_8" // public
#define MQTT_CMD_TOPIC "taist/aiot/command/dev_8"  // subscribe

#define WIFI_SSID "A31A4ED"
#define WIFI_PASSWORD "nice2meetu"
...
void comm_task(void *pvParameter)
{
  ...
  mqtt_client.connect("taist_sound_onsasipat");
  ...
  while (true)
  {
    ...
    Serial.printf("%d, %f, %d\n", millis(), value, detected);
      char payload[200];
  sprintf(payload, "{\"ID\": %s ,\"Detected\":%.d,\"valve\":%.2f}",
